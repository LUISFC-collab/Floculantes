...
const CACHE = 'floculantes-v20260920b15';
const SHELL = ['./', './index.html', './config.js', './manifest.json', './icon-192.png', './icon-512.png'];
const EXTRA = ['./heic2any.min.js']; /* 1,35 MB: se guarda por detras, no retrasa la activacion */

self.addEventListener('install', function (e) {
  self.skipWaiting();
  e.waitUntil(caches.open(CACHE).then(function (c) { c.addAll(EXTRA).catch(function () {}); return c.addAll(SHELL).catch(function () {}); }));
});
