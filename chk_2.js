...
function _crXerMenuUI(b,c,o){
  ...
  menu.innerHTML='<b style="font-size:11px;color:#8ECBF5;letter-spacing:.04em">\u00bfQU\u00c9 .XER QUIERES?</b>'+
    ...
    '<span style="color:#9db4d6">avance hasta ese día inclusive; la fecha de datos es el hábil siguiente y el archivo sale corrido a ella (sin F9)</span></div>'+
    ...
    bt('rep',0,'#1B5E8A','🔁 Para reprogramación · normal',
      ...
      'Entra como <b>proyecto nuevo</b> (-SEMANAn) con la fecha de datos en la mañana hábil siguiente al corte, y P6 reprograma desde ahí lo que falta.')+
...
function _crExportUI(){
  ...
  ov.innerHTML='<div style="background:#fff;width:100%;max-width:560px;border-radius:16px 16px 0 0;padding:17px 16px;box-sizing:border-box;max-height:92vh;overflow:auto">'+
    ...
    '<label style="display:block;font-size:11px;font-weight:800;color:#1F3864;margin-bottom:4px">Avance hasta (corte)</label>'+
    '<input type="date" id="_crExpFec" style="width:100%;box-sizing:border-box;border:1.5px solid #C7D2E0;border-radius:11px;padding:12px;font-size:14px;font-family:inherit;background:#fff;margin-bottom:9px">'+
  ...
  (function(){
    ...
    if(e&&f){
      ...
      e.oninput=_nota;
      f.addEventListener('change',function(){var w=_crXerSemReporte(f.value);if(w>0)e.value=w;_nota()})}
    _nota()})();
  ...
  document.getElementById('_crExpGo').onclick=async function(){
    ...
    try{
      ...
      var c=_crLista().filter(function(x){return x.cron_id===id})[0];
      if(!c)throw new Error('no encuentro ese cronograma');
      var corte=document.getElementById('_crExpFec').value||hoy;
